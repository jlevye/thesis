...
#Read in graph
path = "/home/jen/Documents/School/GradSchool/Thesis/Images/MethodsPaperSamples/SVGsReady/Diversity_graphs/ginkgo.xml.gz"
...
for e in g.edges():
     g.ep.filter[e] = 1
vp, ep = gt.betweenness(g, vprop = g.vp.btwn, eprop = g.ep.btwn, weight = g.ep.vol)

step = 25

##Commenting out random filtering to do descending sort runs for between and weight
# #Get initial values
# n_removed.append(0)
# largest.append(component_size(g, root))
...
# d = {"Removed" : n_removed, "LargestConnected" : largest, "Cost": cost, "Efficiency" : eff}
#
# data_random = pd.DataFrame(d)
# data_random.to_csv("~/Documents/School/GradSchool/Thesis/Data/QFTrandom.csv")
#
...
weight_index = list(np.argsort(g.ep.vol.get_array()))[::-1]
counter = 1
...
data_weight.to_csv("~/Documents/School/GradSchool/Thesis/Data/GFTweightdesc.csv")
...
btwn_index = list(np.argsort(g.ep.btwn.get_array()))[::-1]
counter = 1
for i in btwn_index:
    g.ep.filter.get_array()[i,] = 0
    if i % step == 0:
...
data_btwn.to_csv("~/Documents/School/GradSchool/Thesis/Data/GFTbtwndesc.csv")

#Run iterated version
